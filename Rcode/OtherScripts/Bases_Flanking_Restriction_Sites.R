#JLW -2019
#Finds GC content of bases flanking known restriction sites on genome
...
# ***Before running you must download the assemblies from RefSeq listed in enzymetype_rm$File (REBASEGenomeEnzymes.RData)
# example output in /Data/FlankGCRfast_flankdist1.txt
...
fileFlanks <- function(file,recseq,flankdist){
  ...
  if(!inherits(x,"try-error")){
    write(paste(x$File,
                ...
                sep="\t"),file=paste("FlankGCRfast_flankdist",flankdist,".txt",sep=""),append=T)
  }
  return(x)
}

x <- mcmapply(fileFlanks,frm$File,frm$RecSeqs,MoreArgs=list(flankdist=flank_dist),mc.cores=19)
save(x,file=paste("FlankGCRfast_flankdist",flankdist,".RData",sep=""))
